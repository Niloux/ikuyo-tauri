...
fn main() {
    ...
    // 2. 配置控制台输出
    let console_layer = fmt::layer().with_writer(std::io::stdout).pretty(); // 漂亮的控制台输出

    // 3. 配置文件输出
    let file_layer = fmt::layer().with_writer(non_blocking_file_appender).json(); // JSON 格式的文件输出，方便机器解析

    // 4. 配置环境变量过滤器
    // 允许通过 RUST_LOG 环境变量控制日志级别，例如 RUST_LOG=info,ikuyo_app_lib=debug
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("info")); // 默认日志级别为 info
    ...
    if let Err(e) = ikuyo_app_lib::run() {
        tracing::error!("应用程序发生致命错误: {:?}", e);
        // 在未来的版本中，可以在这里弹出一个对话框通知用户
        std::process::exit(1);
    }
}
