...
impl<'a> AnimeRepository<'a> {
    ...
    pub async fn insert_many_animes(
        ...
        for anime in animes {
            sqlx::query(
                "INSERT OR IGNORE INTO anime (mikan_id, bangumi_id, title, original_title, broadcast_day, broadcast_start, official_website, bangumi_url, description, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            )
            .bind(anime.mikan_id)
