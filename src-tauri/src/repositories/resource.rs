...
impl<'a> ResourceRepository<'a> {
    ...
    pub async fn create(&self, resource: &Resource) -> Result<i64, sqlx::Error> {
    ...
    pub async fn get_by_id(&self, id: i64) -> Result<Option<Resource>, sqlx::Error> {
    ...
    pub async fn list(&self, limit: i64, offset: i64) -> Result<Vec<Resource>, sqlx::Error> {
        if limit > 0 {
            ...
            sqlx::query_as::<_, Resource>("SELECT * FROM resource ORDER BY release_date DESC OFFSET ?")
                .bind(offset)
                .fetch_all(self.pool)
                .await
        }
    ...
    pub async fn update(&self, resource: &Resource) -> Result<(), sqlx::Error> {
        sqlx::query(
    ...
    pub async fn count_by_episode(
        &self,
        mikan_id: i64,
    ) -> Result<Vec<EpisodeResourceCount>, sqlx::Error> {
        sqlx::query_as::<_, EpisodeResourceCount>(
