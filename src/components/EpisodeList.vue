  ...
  <div class="episode-list-container">
    ...
    <div v-if="!loading && !error && episodeStats" class="episode-stats">
      <span class="stats-text">
        共{{ episodeStats.totalCount }}章节，已有{{ episodeStats.availableCount }}章节
        <template v-if="displayMode === 'grid' && needsPagination && currentPageRange">
      ...
      <div class="carousel-wrapper">
        <div class="episode-carousel" ref="carouselContainer">
          <div
            ...
            :key="episode.number"
            :class="['episode-card', episode.available ? 'available' : 'unavailable']"
        ...
        <div class="carousel-controls">
          <button @click="scrollLeft" :disabled="isAtStart" class="control-btn prev-btn">←</button>
          <button @click="scrollRight" :disabled="isAtEnd" class="control-btn next-btn">→</button>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="displayMode === 'grid' && episodes.length > 0">
      <div class="episode-grid" :style="gridStyle">
        <div
          ...
          :key="episode.number"
          :class="['episode-item', episode.available ? 'available' : 'unavailable']"
          @click="handleEpisodeClick(episode)"
      ...
      <div v-if="needsPagination" class="pagination-container">
        ...
        <div class="pagination-controls">
          <button @click="goToPrevPage" :disabled="currentPage === 1" class="pagination-btn prev-btn">上一页</button>
          <div class="page-numbers">
            <button v-if="currentPage > 3" @click="goToPage(1)" class="pagination-btn page-btn">1</button>
            ...
            <button v-if="currentPage < totalPages - 2" @click="goToPage(totalPages)" class="pagination-btn page-btn">{{ totalPages }}</button>
          </div>
          <button @click="goToNextPage" :disabled="currentPage === totalPages" class="pagination-btn next-btn">下一页</button>
        </div>
        <div class="pagination-jump">
          <span>跳转到</span>
          <input type="number" :min="1" :max="totalPages" v-model.number="jumpPage" @keyup.enter="handleJumpToPage" class="jump-input" />
...
const episodeStats = computed(() => {
  ...
  const currentPageAvailableCount = props.displayMode === 'grid' ? currentPageEpisodes.value.filter(ep => ep.available).length : 0
  return {
...
.episode-carousel::-webkit-scrollbar { height: 6px; }
.episode-carousel::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 3px; }
.episode-carousel::-webkit-scrollbar-thumb { background: #888; border-radius: 3px; }
.episode-carousel::-webkit-scrollbar-thumb:hover { background: #555; }
...
@media (max-width: 768px) {
  .episode-list-container { padding: 1.5rem; }
  .episode-card { flex: 0 0 250px; height: 180px; }
  .carousel-controls { display: none; }
  .pagination-controls { gap: 0.25rem; }
}

@media (max-width: 480px) {
  .episode-card { flex: 0 0 220px; height: 160px; }
  .pagination-controls { flex-direction: column; gap: 0.75rem; }
}
</style>
