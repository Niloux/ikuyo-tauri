      ...
      <div class="modal-content scale-in">
        ...
        <form @submit.prevent="onSubmit">
          <div class="form-group" :class="{ 'has-error': errors.mode }">
            ...
            <div v-if="errors.mode" class="error-message">
              {{ errors.mode }}
            </div>
          </div>
          ...
          <div
            class="form-group"
            v-if="task.mode === 'season' || task.mode === 'year'"
            :class="{ 'has-error': errors.year }"
          >
            <label for="year">年份:</label>
            <select
              id="year"
              v-model.number="task.year"
              required
              class="filter-select"
            >
          ...
          <div
            class="form-group"
            v-if="task.mode === 'season'"
            :class="{ 'has-error': errors.season }"
          >
            ...
            <div v-if="errors.season" class="error-message">
              {{ errors.season }}
            </div>
          </div>
          ...
          <div class="form-group" :class="{ 'has-error': errors.limit }">
            ...
            <input
              type="number"
              id="limit"
              v-model.number="task.limit"
              min="1"
              class="filter-input"
            />
            <div v-if="errors.limit" class="error-message">
              {{ errors.limit }}
            </div>
          </div>
...
.modal-fade-leave-active {
  transition:
    opacity 0.25s cubic-bezier(0.4, 1.3, 0.6, 1),
    transform 0.25s cubic-bezier(0.4, 1.3, 0.6, 1);
}
