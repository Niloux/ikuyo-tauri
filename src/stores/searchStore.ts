...
import BangumiApiService, {
  convertSubjectToCalendarItem,
} from "../services/bangumi/bangumiApiService";
...
export const useSearchStore = defineStore("search", () => {
  ...
  const performSearch = async (
    ...
    try {
      ...
      // 搜索获取bangumi_id列表
      const searchData = await BangumiApiService.searchLibrary(
        searchQuery.value,
        page,
        12,
        options,
      ...
      if (searchData.bangumi_ids.length > 0) {
        // 批量获取番剧详情
        const { success: subjects } = await BangumiApiService.batchGetSubjects(
          searchData.bangumi_ids,
        );

        // 转换为AnimeCard兼容格式
        searchResults.value = subjects.map((subject) =>
          convertSubjectToCalendarItem(subject),
        );
      } else {
        searchResults.value = [];
      }
