...
export const useSubscriptionStore = defineStore("subscription", () => {
  ...
  const fetchSubscriptions = async (params: GetSubscriptionsParams = {}) => {
    try {
      ...
      // 获取订阅列表
      const response =
        await subscriptionApiService.getSubscriptions(finalParams);

      // 直接使用缓存的番剧数据构建 subscriptionsWithAnime
      const subscriptionsWithAnime: SubscriptionWithAnime[] =
        response.subscriptions.map((sub) => {
          ...
          return {
            ...
            anime: {
              ...
              rating: sub.anime_rating
                ? { total: 0, count: {}, score: sub.anime_rating }
                : { total: 0, count: {}, score: 0 }, // 提供默认值
              rank: sub.rank || 0, // 提供默认值
              images: sub.images
                ? JSON.parse(sub.images)
                : { large: "", common: "", medium: "", small: "", grid: "" }, // 解析 JSON 字符串
  ...
  const sortSubscriptions = async (
    sort: GetSubscriptionsParams["sort"],
    order: GetSubscriptionsParams["order"] = "desc",
  ) => {
