...
const { handleApiError, showSuccess, handleValidationErrors } =
  useErrorHandler();
...
watch(
  () =>
    (Array.isArray(taskStore.tasks) ? taskStore.tasks : []).map((t) => ({
      id: t.id,
      status: t.status,
    })),
  (newTasks) => {
...
function validateCreateTaskForm() {
  ...
  if (
    (newTask.value.mode === "season" || newTask.value.mode === "year") &&
    !newTask.value.year
  ) {
  ...
  if (
    newTask.value.limit !== undefined &&
    newTask.value.limit !== null &&
    (!Number.isInteger(newTask.value.limit) || newTask.value.limit <= 0)
  ) {
